...
// Supply-specific stages matching the Kanban phases
export type SupplyPropertyStage = 
  | "draft"
  | "in-review"
  | "needs-correction"
  | "in-negotiation"
...
export interface Property {
  ...
  currentStage: SupplyPropertyStage;
...
const STORAGE_KEY = "vistral_supply_properties";
...
export function checkDuplicateProperty(
  ...
  // Active stages (not rejected)
  const activeStages: SupplyPropertyStage[] = [
    "draft",
    "in-review",
    "needs-correction",
    "in-negotiation",
...
function generatePropertyId(): string {
  ...
  const maxId = properties.reduce((max, prop) => {
    const num = parseInt(prop.id.replace("supply_", ""));
    return isNaN(num) ? max : Math.max(max, num);
  }, 0);
  
  return `supply_${(maxId + 1).toString().padStart(7, "0")}`;
}
