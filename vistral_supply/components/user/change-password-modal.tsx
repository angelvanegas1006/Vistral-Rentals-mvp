...
export function ChangePasswordModal({ open, onOpenChange }: ChangePasswordModalProps) {
  ...
  const isDemoMode = () => {
    ...
    return !supabaseUrl || supabaseUrl === 'your_supabase_url_here' || supabaseUrl.trim() === '';
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isDemoMode()) {
      toast.info("Demo mode: Password change is not available. Configure Supabase to enable this feature.");
      onOpenChange(false);
      return;
    }
      ...
      <DialogContent className="w-[95vw] max-w-[425px] max-h-[90vh] overflow-y-auto p-4 sm:p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2 sm:gap-0 pt-4">
            ...
            <Button type="submit" disabled={loading || isDemoMode()} className="w-full sm:w-auto order-1 sm:order-2">
              {loading ? (
                <>
