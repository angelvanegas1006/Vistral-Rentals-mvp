...
interface DniUploadZoneProps {
  ...
  folder?: string; // Folder name in Storage (e.g., "owners", "tenants")
}
...
export function DniUploadZone({
  ...
  const handleRemoveFile = useCallback(async (index: number) => {
    ...
    if (fileToRemove?.path && propertyId) {
      try {
        await deleteFileFromStorage(fileToRemove.path);
      } catch (error) {
        console.error("[DniUploadZone] Error deleting file from Storage:", error);
        // Continue with removal from state even if Storage delete fails
      }
    }
    
    uploadHook.removeFile(index);
  }, [uploadHook, files, propertyId]);

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <Label className="text-sm font-semibold">
          {t.sellerFields.dniNifCif} <span className="text-red-500">*</span>
        </Label>
      ...
      <p className="text-xs text-muted-foreground">
        {t.sellerFields.dniNifCifDescription}
      </p>
      ...
      >
        ...
        <p className="text-xs text-[var(--prophero-gray-500)] dark:text-[var(--prophero-gray-500)]">
          {t.formLabels.maxFilesInfo.replace("{maxFiles}", maxFiles.toString()).replace("{maxSizeMB}", maxSizeMB.toString())}
        </p>
