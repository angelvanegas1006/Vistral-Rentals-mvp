...
interface FileUploadZoneProps {
  ...
  folder?: string; // Folder name in Storage (e.g., "owners", "tenants", "documentation")
}
  ...
  const handleRemoveFile = useCallback(async (index: number) => {
    ...
    if (fileToRemove?.path && propertyId) {
      try {
        ...
        console.error("[FileUploadZone] Error deleting file from Storage:", error);
        ...
        <div className="space-y-2">
          {files.map((file, fileIndex) => (
            ...
            >
              ...
              {/* Show URL if file is from Storage */}
              {file.url && (
                <a
                  href={file.url}
                  target="_blank"
